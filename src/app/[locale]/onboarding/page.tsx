...
function OnboardingForm() {
  ...
  const telegramName = searchParams.get("name") ?? "";
  const telegramUsername = searchParams.get("tgUsername") ?? "";
  const tgPhotoUrl = searchParams.get("photoUrl") ?? "";
  ...
  const onSubmit = async (values: OnboardingValues) => {
    ...
    if (tgPhotoUrl) payload.avatarUrl = tgPhotoUrl;
    if (values.section) payload.section = values.section;
    if (values.track) payload.track = values.track;
    if (values.electiveSubject)
      payload.electiveSubject = values.electiveSubject;
    ...
    try {
      ...
      if (!json.ok) {
        ...
        // A username collision reported by the server invalidates the field.
    ...
    <>
      ...
      <main className="flex flex-1 items-start justify-center px-4 py-16 sm:items-center">
        <div className="w-full max-w-lg">
          <Link
            href="/"
            className="mb-6 inline-flex items-center gap-1.5 text-sm text-muted transition-all duration-200 ease-in-out hover:text-foreground"
          >
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted">
                {telegramId ? t("subtitleWithTelegram") : t("subtitleNoTelegram")}
              </p>
            </CardHeader>
            <CardContent>
              {telegramId && (
              ...
              >
                ...
                <Field>
                  ...
                  <Input
                    ...
                    error={errors.username?.message ?? issues?.username?.[0]}
                  />
                </Field>
                ...
                <Field>
                  ...
                  <Input
                    ...
                    error={errors.password?.message ?? issues?.password?.[0]}
                  />
                </Field>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <Field>
                    ...
                    <Select
                      ...
                      placeholder={
                        watchedYear ? t("systemPlaceholder") : t("systemFirst")
                      }
                      value={watchedSystem}
                      disabled={!watchedYear}
                      onChange={(e) => resetAfterSystem(e.target.value)}
                      error={errors.system?.message ?? issues?.system?.[0]}
                    />
                  </Field>
                </div>
                  ...
                  <>
                    <Field>
                      ...
                      <Select
                        ...
                        error={errors.track?.message ?? issues?.track?.[0]}
                      />
                    </Field>

                    {watchedTrack && (
                      <Field>
                        <Label htmlFor="elective">
                          {t("elective")}
                        </Label>
                        <Select
                          id="elective"
                          ...
                          placeholder={t("electivePlaceholder")}
                  ...
                  sectionOptions.length > 0 && (
                    <Field>
                      ...
                      <Select
                        ...
                        error={errors.section?.message ?? issues?.section?.[0]}
                      />
                    </Field>
                  )
                )}
